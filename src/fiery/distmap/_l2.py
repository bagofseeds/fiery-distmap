...
from ._jit_utils import jitscript, movedim1, square
...
@jitscript
def edt_1d_fillin(f, v, z, w2: float = 1.0):
...
@jitscript
def edt_1d_intersection(f, v, z, k, q: int, w2: float = 1.0):
    vk = v.gather(0, k[None])[0]
    fvk = f.gather(0, vk[None])[0]
    fq = f[q]
    a, b = w2 * (q - vk), q + vk
...
@jitscript
def edt_1d(f, dim: int = -1, w: float = 1.0):
    ...
    k = f.new_zeros(
        f.shape[1:], dtype=torch.long
    )  # index of rightmost parabola in lower envelope
    v = f.new_zeros(
        f.shape, dtype=torch.long
    )  # locations of parabolas in lower envelope
    z = f.new_empty(
        [len(f) + 1] + list(f.shape[1:])
    )  # location of boundaries between parabolas
