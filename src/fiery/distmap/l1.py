...
def l1_distance_transform(
    ...
    vx: Union[float, Sequence[float]] = 1,
) -> Tensor:
    ...
    if backend.jitfields and _jitfields.available:
        return _jitfields.l1_distance_transform(x, ndim, vx)
...
def l1_signed_transform(
    ...
    """
    x = x > 0
    return l1_distance_transform(x, ndim, vx) - l1_distance_transform(
        ~x, ndim, vx
    )
